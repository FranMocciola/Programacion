...
def sigCoeficiente():
    global expoActual, gradoActual
    try:
        coeficiente=float(campo2.get())
        polinomio[expoActual]=coeficiete
        expoActual-=1
        if expoActual>=0:
            actualizarInterfazCoeficiente()
        else:
            finalizarEntradaPolinomio()
    except ValueEror:
        print("Por favor, ingrese un número válido.")


def actualizarInterfazCoeficiente():
    labelCoef.config(text=f"Ingresar el termino del exponente {expoActual}")
    campo2.delete(0,tk.END)


def finalizarEntradaPolinomio():
    LimpiarVentana()
    ############################
    #GRAFICAR
    
def funcionPolinomica2(grado):
    global polinomio, campo2, labelCoef
    expoActual=grado
    polinomio=[0]*(grado+1)
    ...
    labelCoef=tk.Label(ventana, text=f"Ingresar el termino del exponente {i}", font=("Times New Roman", 30),fg="white", bg="#212934")
    labelCoef.place(x=450, y=300)
    campo2=tk.Entry(ventana)
    campo2.place(x=615, y=400,width=200, height=65)
            
    botonOK = tk.Button(ventana, text="OK", font=("Arial",20,"bold"),width=8,bg="#334257",fg="white", command=sigCoeficiente)
